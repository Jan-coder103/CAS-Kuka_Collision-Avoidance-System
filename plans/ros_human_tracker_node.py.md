# ROS 2 Node Wrapper Implementation

**File**: `ros_human_tracker_node.py`

This file contains the complete ROS 2 node wrapper implementation.
...
1. **Wrapper Pattern**: Wraps existing `ObjectDetectionApp` without modifying its core logic
2. **Callback Integration**: Uses `on_coords_callback` to receive coordinate updates
3. **Threading**: Runs ROS 2 in a daemon thread alongside the blocking GUI mainloop
4. **Statistics**: Tracks publish count and timing
5. **Graceful Shutdown**: Proper cleanup when window is closed
...
- Callback is wrapped in try-except for safety
